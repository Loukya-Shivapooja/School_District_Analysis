...
* `% Passing Math score` and `% Passing Reading`  for Thomas High School came considerable down from `93.27%` to `66.91%` and `97.31%` to `69.66%` respectively.
* `% Overall passing` students for Thomas High School came down from `90.95%` to `65.08%`.
### Replacing the ninth graders’ math and reading scores of Thomas High School’s, analysing performance relative to the other schools.
Initial Analysis
...
Considering 9th-12th Grade Data
...
considering 10th-12th Passing Data
